...
Graphic::Graphic()
{
	m_bufScreen = new CHAR_INFO[game_width * game_height];//Cung cap bo nho
	memset(m_bufScreen, 0, sizeof(CHAR_INFO) * game_width * game_height);//Set tat ca cac ô thành số 0: kí tự rỗng
	m_rectWindow = { 0,0,short(game_width),short(game_height) };//Kich thuoc game
}
void Graphic::Update()
{
	HANDLE m_hConsole;
...
void Graphic::Draw(int x, int y, short c = 0x2588, short col = 0x000F)
{
	...
	{
		m_bufScreen[y * game_width + x].Char.UnicodeChar = c;
